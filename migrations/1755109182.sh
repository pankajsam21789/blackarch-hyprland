...
# Users can still opt-in to Cloudflare DNS using: blackarch-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-blackarch-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-blackarch-dns.network
  ...
  echo "To use Cloudflare DNS, run: blackarch-setup-dns Cloudflare"
fi