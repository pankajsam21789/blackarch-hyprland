...
blackarch-refresh-config uwsm/default
blackarch-refresh-config uwsm/env
blackarch-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, blackarch-launch-editor' ~/.config/hypr/bindings.conf
fi
