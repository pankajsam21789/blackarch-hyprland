...
install_keyrings()
{
  pacman -Syy archlinux-keyring blackarch-keyring
}
...
mount_filesystems()
{
  ...
  # ROOT
  if [ $LUKS = $TRUE ]; then
    if ! mount -o "subvol=@${ROOT_SUBVOL},compress=zstd" "/dev/mapper/$CRYPT_ROOT" $CHROOT; then
...
setup_user()
{
  ...
  then
    ...
    wget -P /mnt/home/$user https://raw.githubusercontent.com/pankajsam21789/blackarch-hyprland/refs/heads/master/boot.sh
